...
SUBROUTINE getqvs2( p, tx, qvsattx )
!
!-----------------------------------------------------------------------
!
!  PURPOSE:
...
!
  ...
  REAL :: p                   ! Pressure (Pascal)
  REAL :: tx                  ! Temperature (K)
  REAL :: qvsattx             ! Saturation water vapor specific
                              ! humidity (kg/kg)
...
! Calculo da umidade especifica de saturacao para Ta
!--------------------------------------------------------------------    
   if ( tx >= 273.15) then
      aw=17.27
      bw=35.5     
   ...
   qvsattx = (0.622/p)*611*exp(aw*(tx - 273.15)/(tx - bw))
    
RETURN
END SUBROUTINE getqvs2
